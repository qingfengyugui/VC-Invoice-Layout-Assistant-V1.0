...
Install this folder in the Claude Code filesystem Skill root (`.claude/skills` or `~/.claude/skills`) and add the local stdio service with `claude mcp add --transport stdio invoice-layout -- invoice-layout mcp`. Do not use the internal Work Buddy gateway for this service.
